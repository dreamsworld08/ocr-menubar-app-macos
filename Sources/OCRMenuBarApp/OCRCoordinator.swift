...
final class OCRCoordinator {
    ...
    private init() {
        overlayController.onSelection = { [weak self] selection in
            self?.performOCR(for: selection)
        }
    }
    ...
    private func performOCR(for rect: CGRect) {
        Task.detached(priority: .userInitiated) { [weak self] in
            guard let image = ScreenCapture.capture(rect: rect) else {
                await MainActor.run { self?.showError(message: "Could not capture the selected area.") }
                return
            }
    ...
    private func showPermissionHint() {
        ...
        alert.informativeText = "Enable this app in System Settings > Privacy & Security > Screen Recording, then try Start OCR again."
